...
public class BitmapUtils {
     ...
     */
    public static Bitmap compressImage(Bitmap image, double size, int options) {
     ...
     * @return 原路返回图片
     */
    public static File compressImage(Bitmap image, double size, int options, File file) {
    ...
    public static String saveBitmap(Bitmap bitmap, String fileName) {
        ...
        File catalog = new File(Constants.Companion.getPICTRUE_PATH());
        if (!catalog.exists()) {
            catalog.mkdirs();
        }

        File file = new File(catalog, fileName.replace(".jpg", "new.jpg"));
        if (file.exists()) {
            file.delete();
        }
        FileOutputStream fileOutputStream = null;
...
//        }
     ...
     * @param data
     */
    public static File saveCameraPhoto(byte[] data) {
        File dirfile = new File(Constants.Companion.getPICTRUE_PATH());
        if (!dirfile.exists()) {
            dirfile.mkdirs();
        }
        ...
        options.inPreferredConfig = Bitmap.Config.RGB_565;//据说效率还可以
